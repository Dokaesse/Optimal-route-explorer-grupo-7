...
class Graph:
    ...
    def find_shortest_path(self, origin, destination):
        ...
        # Buscamos los nodos de origen y destino en la lista de nodos
        for node in self.nodes:
            if node.name == origin:
                origin_node = node
        ...
        # Si no se encuentran ambos nodos, terminamos la función
        if not fin:
        ...
        # Bucle principal: continúa hasta encontrar el destino
        ...
        fig, ax = last_path.path_plot(self)  # Graficamos el camino encontrado

        self.paths.clear()  # Limpiamos la lista de caminos para futuras búsquedas
        return fig, ax  # Devolvemos la figura y los ejes de la gráfica
